...
%   @output dH_i: jacobian of h_i evaluated at(x_i, u_i, x_ip1, u_ip1).
  ...
  h_i = dynamics_constraint(x_i, u_i, x_ip1, u_ip1, dt);
  ...
  for j=1:numel(x_i)
      ...
      dx(j) = delta;
      dHx_i_j = dynamics_constraint(x_i + dx, u_i, x_ip1, u_ip1, dt) - h_i;
      dHx_ip1_j = dynamics_constraint(x_i, u_i, x_ip1 + dx, u_ip1, dt) - h_i;
  ...
  for j=1:numel(u_i)
      ...
      du(j) = delta;
      dHu_i_j = dynamics_constraint(x_i, u_i + du, x_ip1, u_ip1, dt) - h_i;
      dHu_ip1_j = dynamics_constraint(x_i, u_i, x_ip1, u_ip1 + du, dt) - h_i;
